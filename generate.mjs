 ...
 *
 * Env vars:
 *   GH_USERNAME  - GitHub login to fetch contributions for (default: 1iki)
...
const USERNAME = process.env.GH_USERNAME || "1iki";
...
async function fetchWeeks() {
  if (!TOKEN) {
    console.warn("No GH_TOKEN or GITHUB_TOKEN found. Using fallback mock/public calendar generator if unauthenticated request fails.");
  }
  const headers = {
    "Content-Type": "application/json",
    "User-Agent": "1iki-jet-heatmap",
  };
  if (TOKEN) {
...
function pickTargets(cells) {
  ...
  if (activeCells.length === 0) {
    // If user has no active days in window, pick a few distributed cells to keep animation lively
    return [cells[10], cells[45], cells[100], cells[180], cells[220]].filter(Boolean);
  }
  return [...activeCells]
...
async function main() {
  ...
  try {
    ...
    console.error("GraphQL API request failed:", err.message);
    console.log("Generating fallback calendar grid...");
    // Fallback calendar grid (34 weeks x 7 days)
    weeks = Array.from({ length: COLS }, (_, w) => ({
      contributionDays: Array.from({ length: ROWS }, (_, d) => ({
        contributionCount: (w + d) % 3 === 0 ? (w % 5) + 1 : 0,
        color: (w + d) % 3 === 0 ? "#216e39" : "#161b22",
        date: `2026-01-01`,
      })),
    }));
  }
